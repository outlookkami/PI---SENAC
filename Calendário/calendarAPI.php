...
class GoogleCalendarApi{
    ...
    const CALENDAR_EVENTS = 'https://www.googleapis.com/calendar/v3/calendars/';
    ...
    public function GetAccessToken($client_id, $redirect_uri, $client_secret, $code){
       $curlPost = 'client_id=' . $client_id . '$redirect_uri=' . $redirect_uri . '$client_secret=' . $client_secret . '$code=' . $code . '$grant_type=authorization_code';
        ...
        if($http_code != 200){
            $error_msg = 'Falha ao obter o token de acesso';
            if(curl_errno($ch)){
    ...
    public function GetUserTimezone($access_token){
        $ch = curl_init();
        ...
        $http_code =  curl_getinfo($ch, CURLINFO_HTTP_CODE);
        if($http_code != 200){
            $error_msg = 'Falha ao obter o fuso horário do usuário';
            if(curl_errno($ch)){
                $error_msg = curl_error($ch);
            }
            throw new Exception('Error '.$http_code.': '.$error_msg);
    ...
    public function GetCalendarList($access_token){
        ...
        if($http_code != 200){
            $error_msg = 'Falha ao obter a lista de calendários do usuário';
            if(curl_errno($ch)){
                $error_msg = curl_error($ch);
            }
            throw new Exception('Error '.$http_code.': '.$error_msg);
    ...
    public function CreateCalendarEvent($access_token, $calendar_id, $event_data, $all_day, $event_datetime, $event_timezone){
        ...
        if($all_day == 1){
            ...
            $curlPost['end'] = array('dateTime' => $dateTime_end, 'timeZone
            ' => $event_timezone);
        }
        ...
        if($http_code != 200){
            $error_msg = 'Falha ao criar o evento no calendário';
            if(curl_errno($ch)){
                $error_msg = curl_error($ch);
            }
            throw new Exception('Error '.$http_code.': '.$error_msg);
    ...
    private function getTimezoneOffset($timezone = 'America/Brasilia'){
        $current_timezone = timezone_open($timezone);
        $utcTime = new \DateTime('now', new \DateTimeZone('UTC'));
        $offsetInSecs = timezone_offset_get($current, $utcTime);
        $hoursAndSec = gmdate('H:i', $offsetInSecs);
